...
class EventsController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	...
	public function listAction() {
		...
		$json = @file_get_contents($apiUrl . 'Events/GetBookableItemList/' . $authCode);
		...
		$jsonBookableItems = @file_get_contents($apiUrl . 'Events/GetAvailability/' . $authCode . '/' . $startDate . '/' . $endDate . '/?bookableItemId=' . $bookableItemId);
		...
		$jsonValidate = @file_get_contents($apiUrl . 'ValidateAccount/' . $authCode);
		...
		$Booking = array("ApiToken" => $authCode, "EventIdentifier" => $bookableItems[0][EventIdentifier], "BookableItemId" => $bookableItems[0][EventIdentifier][BookableItems][Id], "EventScheduleId" => $bookableItems[0][EventScheduleId], "Courtesy" => "Mr.", "FirstName" => "Ta", "LastName" => "Kov", "PhoneNumber" => "+359888888888", "Email" => "soft_tech@abv.bg", "Description" => "I like this event.");
        $json_data = json_encode($Booking);
        $post = file_get_contents(apiUrl . 'Events/SetBooking', null, stream_context_create(array(
                        'http' => array(
        ...
        $setBookingConfirmation = json_decode($post, true);
